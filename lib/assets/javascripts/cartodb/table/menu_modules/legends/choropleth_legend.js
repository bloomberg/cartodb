...
cdb.admin.mod.ChoroplethLegend = cdb.admin.mod.CustomLegend.extend({
  ...
  _calculateItems: function() {
    ...
    if (customColorRamps) {
      var colorRampIndex = this.wizardProperties.get('color_ramp');
      if (!customColorRamps.hasOwnProperty(colorRampIndex)) {
        colorRampIndex = Object.keys(customColorRamps)[0];
      }
      ramp = customColorRamps[colorRampIndex];
